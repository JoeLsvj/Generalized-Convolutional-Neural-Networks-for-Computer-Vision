...
class CharactersDataset(torch.utils.data.Dataset):
    def __init__(self, root_dir, transform=None):#classes,
...
def main(argus):
    ...
    # datasets
    trainset = CharactersDataset(root_dir=root_dir, transform=transform)
    testset = CharactersDataset(root_dir=root_dir, transform=transform)
    ...
    recurrences = torch.Tensor(list(trainset.recurrences.values())).to(device)
    ...
    while t < epoch:
        ...
        model.eval()
        current_accuracy = get_accuracy(testloader=testloader, model=model)
...
if __name__ == '__main__':
    ...
    ap.add_argument("-mr", "--max_rotation", type=int, required=False,
                 help="massimo angolo di rotazione casuale per l'augmentation del dataset", default=0)
